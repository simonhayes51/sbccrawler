...
async def find_real_solution_urls():
    ...
    try:
                ...
                try:
                    ...
                    for pattern in solution_patterns:
                        for match in re.findall(pattern, response.text):
                            url = "https://www.fut.gg" + match if match.startswith("/") else match
                            if "squad-builder" in url and url not in [s["url"] for s in found_solutions]:
                                found_solutions.append({"url": url, "found_on": sbc_url, "pattern": pattern})

                    for link in soup.find_all("a", href=True):
                        href = link["href"]
                        if "/25/" in href and "squad-builder" in href:
                            url = "https://www.fut.gg" + href if href.startswith("/") else href
                            if url not in [s["url"] for s in found_solutions]:
                                found_solutions.append({"url": url, "found_on": sbc_url, "pattern": "href_attribute"})
                except Exception as e:
                    print(f"Error checking {sbc_url}: {e}")

        # Test discovered URLs with a fresh client
        working_urls = []
        import re as _re  # ensure in scope
        async with httpx.AsyncClient() as client:
            for solution in found_solutions[:10]:
                try:
                    html = (await client.get(solution["url"], headers=headers, timeout=30)).text
                    pattern_results = {}
                    patterns = [
                    ...
                    for name, pattern in patterns:
                        ...
                        if matches:
                            ...
                            pattern_results[name] = {"count": len(uniq), "sample_ids": uniq[:5]}

                    if any(pr.get("count", 0) > 0 for pr in pattern_results.values()):
                        working_urls.append({
                            **solution,
        ...
        return {
            ...
            "tested_solutions": min(10, len(found_solutions)),
            "working_solutions": len(working_urls),
            "found_solution_urls": [s["url"] for s in found_solutions[:20]],
            "working_solution_details": working_urls,
            "recommendation": (
                f"Found {len(working_urls)} working solution URLs with player IDs"
                if working_urls else
                "No working solution URLs found - may need different approach"
            ),
        }
    except Exception as e:
        import traceback
...
async def test_raw_html_extraction():
    ...
    try:
        ...
        for pattern in card_id_patterns:
            card_id_matches.extend(re.findall(pattern, html_content, re.IGNORECASE))
        ...
        for pattern in img_patterns:
            img_matches.extend(re.findall(pattern, html_content, re.IGNORECASE))
        unique_img_matches = sorted(set(img_matches))

        return {
            "status": "success",
            "test_url": test_url,
            "html_length": len(html_content),
            "webp_pattern_matches": {"count": len(unique_matches), "player_ids": unique_matches[:10]},
            "webp_specific_matches": {"count": len(unique_webp_matches), "player_ids": unique_webp_matches[:10]},
            "general_25_matches": {"count": len(unique_general_matches), "player_ids": unique_general_matches[:10]},
            "any_25_occurrences": {"count": len(unique_any_25), "examples": unique_any_25[:10]},
            "card_id_json_matches": {"count": len(unique_card_ids), "card_ids": unique_card_ids[:10]},
            "image_url_matches": {"count": len(unique_img_matches), "player_ids": unique_img_matches[:10]},
            "html_samples": {
...
async def test_solution_extraction():
    ...
    try:
        ...
        # Try multiple test URLs; may be replaced by discovered ones
        test_urls = [
        ...
        # Attempt to harvest some live solution URLs from an SBC page
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if found_solution_urls:
                    test_urls = found_solution_urls[:3]
        except Exception:
        ...
        for test_url in test_urls:
            try:
                ...
                if valid_ids:
                    # Optional DB lookup if your db module is present
                    try:
        ...
        return {
            ...
            "url_results": results,
            "debug_info": {
                "extraction_pattern": r'25-(\d+)\.([\w]+)\.webp',
